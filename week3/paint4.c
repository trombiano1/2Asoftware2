...
int main(int argc, char **argv)
{
  ...
  int width;
  int height;
  char filename[100] = "history.txt";
  int fillmode = 0;
  if (argc != 3 && argc != 4 && argc != 5){
    fprintf(stderr,"usage: %s <width> <height> <filename>\nor\n%s <width> <height>",argv[0],argv[0]);
    return EXIT_FAILURE;
  } else {
    
    if(argc == 4 && strcmp(argv[3], "fillmode") == 0){
      fillmode = 1;
    } else if (argc == 4){ 
    ...
    if(argc == 5 && strcmp(argv[4], "fillmode") == 0){
      fillmode = 1;
      strcpy(filename, argv[3]);
    } else {
      strcpy(filename, argv[3]);
...
Canvas *init_canvas(int width,int height, char pen)
{
  ...
  char *tmp2 = (char *)malloc(width*height*sizeof(char));
  memset(tmp, ' ', width*height*sizeof(char));
  for (int i = 0 ; i < width ; i++){
    new->canvas[i] = tmp + i * height;
    new->colors[i] = tmp2 + i * height;
  }
...
void reset_canvas(Canvas *c)
{
  ...
  memset(c->colors[0], ' ', width*height*sizeof(char));
}
...
void print_canvas(FILE *fp, Canvas *c)
{
  ...
  char **colors = c->colors;
  char pen = c->pen;
  int fillmode = c->fillmode;
  ...
  if(fillmode == 1){
	...
	const char co = colors[x][y];
	/* fputc(c, fp); */
	if(co == 'l'){
	  fprintf(fp,"\e[34m\x1b[44m");
	}
	if(co == 'r'){
	  fprintf(fp,"\e[31m\x1b[41m");
	}
	if(co == 'g'){
	  fprintf(fp,"\e[32m\x1b[43m");
	}
	if(co == 'y'){
	  fprintf(fp,"\x1b[33m\x1b[43m");
	}
	if(co == 'b'){
	  fprintf(fp,"\x1b[40m");
	}
...
void color_rect(Canvas *c, const int x0, const int y0, const int rect_width, const int rect_height)
{
  for(int i = y0; i < y0+rect_height+1; i++){
    draw_line(c, x0, i, x0+rect_width, i);
  }
}
...
void color_circle(Canvas *c, const int x0, const int y0, const int r)
{
  char pen = c->pen;
  char color = c->color;
  for(int x = 0; x < c->width; x++){
    for(int y = 0; y < c->height; y++){
      if(sqrt((x-x0)*(x-x0)+4*(y-y0)*(y-y0)) < r+0.6){
	c->canvas[x][y] = pen;
	c->colors[x][y] = color;
      }
    }
...
void change_pen(Canvas *c, const char penshape)
{
  c->pen = penshape;
}

void change_color(Canvas *c, const char pencolor)
{
  c->color = pencolor;
}
