...
void my_rand_init_cells(const int height, const int width, int cell[height][width], FILE* fp){
  if(fp == NULL){ //random when no file is assigned
...
int main(int argc, char **argv)
{
  ...
  if ( argc > 2 ) {
    ...
    if ( (lgfile = fopen(argv[1],"r")) != NULL ) {
      my_rand_init_cells(height,width,cell,lgfile); // ファイルによる初期化
    }
    else{
    ...
    my_rand_init_cells(height, width, cell, NULL); // デフォルトの初期値を使う
  }
