...
double calculate_probability(double n, double m, double T);
...
double solve(const City *city, int n, int *route, int *visited, int y)
{
  ...
  for(int a = 0; a < tries; ++a){
	...
	sum_d += distance(city[c0],city[c1]);
      ...
      /* printf("a:%d best:%f sum:%f prob:%f dif:%f T:%f",a,best_routes_sum_d[a],sum_d,calculate_probability(sum_d, best_routes_sum_d[a], T),sum_d-best_routes_sum_d[a],T); */
      if(sum_d < best_routes_sum_d[a]){
	best_routes_sum_d[a] = sum_d;
	for(int b = 0; b < n; ++b){
	  the_flag = 1;
	  best_routes[a*n + b] = temp_route[b];
	...
	if(calculate_probability(sum_d, best_routes_sum_d[a], T) > (double)rand()/RAND_MAX){
	  best_routes_sum_d[a] = sum_d;
	  for(int b = 0; b < n; ++b){
	    the_flag = 1;
	    best_routes[a*n + b] = temp_route[b];
